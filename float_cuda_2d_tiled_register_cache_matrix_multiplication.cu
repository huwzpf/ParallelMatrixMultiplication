...
__global__ void matrixMultiplyKernel(float *A, float *B, float *C, int rowsA, int colsA, int colsB) {
    ...
    __shared__ float As[BLOCK_SIZE * BLOCK_STEP];
    __shared__ float Bs[BLOCK_STEP * BLOCK_SIZE];
    float results[TILE_SIZE * TILE_SIZE] = {0.0}; 

    float aCol[TILE_SIZE ] = {0.0}; 
    float bRow[TILE_SIZE] = {0.0}; 
